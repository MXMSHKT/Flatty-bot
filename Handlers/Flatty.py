...
async def savetoDB(user_data):
    ...
    tdb.update_one({'_id': user_data['id']},
                   {'$set': {'low_price': int(user_data['price'].split("-")[0].strip()),
                             'high_price': int(user_data['price'].split("-")[-1].strip()),
                             'type': user_data['type'],
                             'low_area': float(user_data['area'].split("-")[0].strip()),
                             'high_area': float(user_data['area'].split("-")[-1].strip())
                             }
                    })
...
async def show(message: types.Message, state: FSMContext):
    ...
    data = col.find(
            {'$where': f'this.FlatParams.Total_area >= {low_area}' and f'this.FlatParams.Total_area <= {high_area}'
                       and f'this.Price >= {low_price}' and f'this.Price <= {high_price}',
    ...
    for i in random.choices(data_list,k=4):
        photo_list = []
        for filename in os.listdir(f"photos/{i['_id']}"):
            photo_list.append(str(filename))
        images = random.choices(photo_list[:10], k=3)
        for image in images:
            try:
                await bot.send_photo(message.from_user.id, photo=open(f"photos/{i['_id']}/{image}",'rb'))
            except e:
                await message.answer(no_image)
        msg = flat_msg(i)
        await message.answer(msg, parse_mode="Markdown", disable_web_page_preview=True)

    # await state.set_state(System.show_state)
...
def register_message_handlers(dp: Dispatcher):
    ...
    # dp.register_message_handler(show_next, state=System.show_state)

    dp.register_message_handler(send_price, commands=['price'])
    dp.register_message_handler(set_price, state=Sorting.price_sort)

    dp.register_message_handler(send_area, commands=['area'])
    dp.register_message_handler(set_area, state=Sorting.area_sort)

    dp.register_message_handler(send_type, commands=['type'])
    dp.register_message_handler(set_type, state=Sorting.type_sort)

    # dp.register_message_handler(set_price_incorrect, state=Sorting.price_sort)
